...
DATA_PATH = Path(__file__).parent / "loan_book.csv"
...
@st.cache_data
def load_raw():
    """Return the unmodified CSV for data-quality comparisons."""
...
def load_and_engineer():
    ...
    # Missing delinquency = never delinquent - key insight
...
def compute_woe_iv(df, feature, target="default_flag", bins=10):
    ...
    # Use categorical treatment for low-cardinality integers (binary flags, small counts)
    # to avoid qcut collapsing all boundaries into a single bin and returning IV=0.
    if d[feature].dtype in ["float64", "int64"] and n_unique > bins:
        try:
...
with tabs[0]:
    ...
    fig_miss = px.bar(
        mdf, x="Column", y="Missing %", color="Missing %",
    ...
    with col3: card("DD-Mon-YYYY",  "12,119", "10.0% - e.g. 14-Jan-2021", "orange")
    warn(
        "The 24,002 slash-format dates mix two conventions. Where the first number is 13-31, "
        "it must be DD/MM/YYYY (3,632 unambiguous cases). Where it is 1-12, month and day "
        "are ambiguous. Our parser tries MM/DD first then falls back to DD/MM, resolving all "
        "120,960 dates with zero parsing failures."
    )
    ...
    with col2:
        insight(
            "All other numeric ranges appear plausible: age 21-70, income/loan amounts "
...
with tabs[1]:
    ...
    with col1:
        selected = st.selectbox(
            "Select feature:", list(NUMERIC_FEATURES.keys()),
    ...
    if iv < 0.02:
        iv_str, iv_col = "Useless (<0.02)", "red"
    elif iv < 0.1:
        iv_str, iv_col = "Weak (0.02-0.1)", "orange"
    elif iv < 0.3:
        iv_str, iv_col = "Medium (0.1-0.3)", "blue"
    else:
        iv_str, iv_col = "Strong (>0.3)", "green"
    ...
    fig_iv = px.bar(
        iv_board, x="IV", y="Feature", orientation="h",
...
with tabs[3]:
    ...
    fig_roc.add_trace(go.Scatter(
        ...
        name=f"Our Model (AUC={models['impr_test_auc']:.4f})",
    ...
    fig_coef.update_layout(
        title="Logistic Regression Coefficients (top 20)",
    ...
    eng_table = pd.DataFrame([
        ...
        ["Delinquency rate (delinquencies / accounts)",         "Normalises for portfolio size; 2 delinquencies on 2 accounts > 2 on 20",               "Better risk signal"],
        ["High utilisation flag (≥70%)",                       "Non-linear relationship - risk spikes above 70% threshold",                             "Captures threshold effect"],
        ["Cap credit utilisation at 100%",                     "26 rows above 100% are data errors (34.6% default rate); capping prevents distortion",  "Data quality fix"],
        ...
        ["Parse 3 mixed date formats → month/quarter",           "3 formats: YYYY-MM-DD, slash (MM/DD + DD/MM mixed), DD-Mon-YYYY; 0 unparsed rows", "Data quality + new features"],
...
Our model Gini = **{gini:.3f}** (vs 0.36 baseline).
...
with tabs[5]:
    ...
    with col1:
        insight(
            "**Conservative policy (threshold ~0.10):** ~52% approval, ~4.5% bad rate, ~85% of bad loans "
            "rejected. Best for risk-averse portfolios where loss avoidance outweighs volume."
        )
    with col2:
        insight(
            "**Growth policy (threshold ~0.25):** ~80% approval, ~8.6% bad rate, ~55% of bad loans "
            "rejected. Best when margins are healthy and volume matters more than minimising defaults."
        )
...
| Threshold | Approval Rate | Bad Rate | Bad Loans Avoided |
|---|---|---|---|
| 0.10 | 52% | 4.5% | 85% |
| **0.12** | **58%** | **5.1%** | **81%** |
| **0.15** | **65%** | **6.1%** | **74%** |
...
At threshold 0.15 you approve 65% of applicants while reducing the bad rate from 15.4% to 6.1%
— a 60% reduction in defaults vs approving everyone. The 74% of bad loans avoided at this
threshold represents substantial loss prevention for a retail lender.

We recommend **A/B testing threshold 0.15** against the current approval policy to measure
actual P&L impact before full deployment.
